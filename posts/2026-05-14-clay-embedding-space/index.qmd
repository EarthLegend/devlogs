...
![Query: a Central Valley orchard block, mostly bare soil. Candidates include three forested chips, a two-fields agricultural scene, and a mature bushy orchard. Clay scores the mature orchard most similar.](round_orchard.jpg)
...
The build story (mountain walks, MCP-assisted spec arguments, Claude Code dispatch and dogfooded edges) is over on [the main blog]({MAIN_POST_URL}). This post is about what playing the game actually teaches.
...
![Query: a straight canal across arid ground with a dirt track and scattered trees. Clay finds the candidate that matches it almost component for component.](round_canal.jpg)
...
![Query: dense woodland with scattered houses and a couple of roads. Clay favours the candidate with patchier woodland, distinct roads, and houses among the trees over the visually similar but human-empty forest.](round_forest_houses.jpg)
...
![Query: arid scrubby terrain with no obvious infrastructure. The candidates are all similarly scrubby; the winner is distinguished only by a faint trail running through it.](round_scrubby_trail.jpg)
...
![Query: shrubland on undulating terrain with a dirt track. Tempting to pick the candidate with the most similar road; Clay weights terrain pattern higher and goes for a different chip.](round_semi_desert.jpg)
